...
export const sendPasswordResetEmail = async ({ to, firstName, resetUrl }: PasswordResetEmailArgs) => {
  try {
...
export const sendWelcomeEmail = async ({ to, firstName }: WelcomeEmailArgs) => {
  try {
    await getResend().emails.send({
      from: process.env.EMAIL_FROM || "247Market <onboarding@resend.dev>",
      to,
